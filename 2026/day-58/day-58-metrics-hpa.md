...
**Verify:** Which pod is using the most CPU right now?
   ...
   **kube-apiserver-practice-cluster-control-plane** pod running in ns `kube-system`
...
**Verify:** What does the TARGETS column show?
   * the TARGETS column shows the current resource usage vs. the desired target for autoscaling.
...
5. Scale-down is slow (5-minute stabilization window) — you do not need to wait
   ...
   * Scaled UP & Down automatically

   ![snapshot](images/5-a.png)

   * Pods created and terminated only 1 pod remain (min=1)
...
**Verify:** What does the `behavior` section control?
   * Behavior section control how to scalup and scaledown.
   * `Stabilization windows` : How much to wait before scaling UP/DOWN (e.g., 300 seconds 
      before scale‑down).
   * `Policies` : Specify the maximum allowed scaling rate
      - Percent → Scale relative to current replicas (e.g., 100% means double or halve).  
      - Pods → Scale by a fixed number of pods (e.g., +4 pods).
   * `PeriodSeconds` : Minimum interval between scaling actions 
...
- What the Metrics Server is and why HPA needs it
   * `Metrics Server` collects resource usage data (CPU, memory) from Kubelets across the 
     cluster and exposes it via the Kubernetes API.
   * `HPA` queries this API to get current usage metrics and then decides whether to 
      scale pods up or down.
   * Without Metrics Server, the HPA has no data to base scaling decisions on.

- How HPA calculates desired replicas
   * `desiredReplicas = ceil(currentReplicas * (currentUsage / targetUsage))`

- The difference between `autoscaling/v1` and `v2`
   * `autoscaling/v1` =  Supports scaling only on CPU utilization.
   * `autoscaling/v2` = Supports CPU, memory, custom metrics, and external metrics, plus
      advanced features like the behavior section for fine‑tuning scale‑up/scale‑down policies.
