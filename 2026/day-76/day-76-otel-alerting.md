...
**Document:** What is the difference between Prometheus alerts and Grafana alerts? When would you use each?
   - **Prometheus alerts**
      - Rules added in config file(yaml).
      - Promql expressions only. 
      - Alerts can be set on metrics only.
      - Alertmanager (separate service).
      - Use when needs metrics of CPU, memory, request rates, error rates, etc.
      - Better for large scale infrastructure.
   - **Grafana alerts**
      - Rules added in grafana database configured using UI.
      - Queries from any data source (Prometheus, Loki, MySQL, etc.).
      - Built-in Alertmanager.
      - Use for multi-source alerting.
